...
static double Colour_Tolerance = 0.01;
...
GdkRGBA *get_colour_under_cursor(Display *display) {
  Window root, child;
  ...
  if (!XGetWindowAttributes(display, child, &child_attr)) {
    fprintf(stderr, "Failed to get window attributes\n");
    return NULL;
  }

  int x_offset = win_x - child_attr.x;
  int y_offset = win_y - child_attr.y;

  XImage *image =
      XGetImage(display, child, x_offset, y_offset, 1, 1, AllPlanes, ZPixmap);
  if (!image) {
  ...
  GdkRGBA *current_colour = malloc(sizeof(GdkRGBA));
  if (!current_colour) {
    fprintf(stderr, "Failled to malloc current_colour\n");
    return NULL;
  }
  current_colour->red = (double)r / 255.0;
  current_colour->green = (double)g / 255.0;
  current_colour->blue = (double)b / 255.0;
  current_colour->alpha = 1.0;

  return current_colour;
}

gboolean check_colour_under_cursor(struct colour_opts *data) {
  if (!data || !data->display || !data->colour) {
    fprintf(stderr, "Invalid input data\n");
    return False;
  }

  Display *display = data->display;
  GdkRGBA *target_colour = data->colour;

  GdkRGBA *current_colour = get_colour_under_cursor(display);
  if (!current_colour) {
    return False;
  }

  gboolean is_within_tolerance =
      (fabs(current_colour->red - target_colour->red) < Colour_Tolerance &&
       fabs(current_colour->green - target_colour->green) < Colour_Tolerance &&
       fabs(current_colour->blue - target_colour->blue) < Colour_Tolerance);
  free(current_colour);
...
void click_handler(gpointer *data) {
  ...
  struct colour_opts colour_args = {display, &args->colour};
  ...
  while (isClicking) {
    ...
    // if (args->check_colour) {
    //   g_timeout_add(10000, check_colour_under_cursor, &colour_args);
    // }

    if (args->check_colour) {
      if (!check_colour_under_cursor(&colour_args)) {
    ...
    case CLICK_TYPE_HOLD:
      if (is_holding ==
          FALSE) // Don't re-send mouse_down if already successfully sent
      {
    ...
    if (args->random_interval) {
      ...
      usleep(
          random_between((lower > 0) ? lower : 0, sleep + random_interval_ms));
    } else
      usleep(sleep);
  ...
  if (args->click_type == CLICK_TYPE_HOLD) {
    if (mouse_event(display, args->button, config->use_xevent,
                    MOUSE_EVENT_RELEASE) == FALSE)
      xapp_error("Sending mouse down", -1);
  }
 ...
 */
gboolean toggle_get_active(GtkWidget *button) {
  ...
  case TRUE:
    ...
    gtk_button_set_label(GTK_BUTTON(button), "Click");
    gtk_widget_set_sensitive(button, FALSE);
    break;
  case FALSE:
    gtk_window_set_keep_above(GTK_WINDOW(mainappwindow.pwin), FALSE);
    gtk_button_set_label(GTK_BUTTON(button), "Get");
...
void get_cursor_pos_click_handler_colour(GtkWidget *button) {
  Display *display = get_display();
  mask_config(display, MASK_MOUSE_PRESS);
  ...
  // Convert RGB to GdkRGBA
  GdkRGBA *rgba = get_colour_under_cursor(display);
  if (rgba) {
    gtk_color_chooser_set_rgba(mainappwindow.colour_entry, rgba);
    free(rgba);
  }

  XCloseDisplay(display);
  g_idle_add(toggle_get_active, button);
}
 ...
 */
void get_cursor_pos_handler(GtkWidget *button) {
  Display *display = get_display();
  while (isChoosingLocation) {
    int i_cur_x, i_cur_y;
...
void repeat_only_check_toggle(GtkToggleButton *check) {
  gtk_widget_set_sensitive(mainappwindow.repeat_entry,
                           gtk_toggle_button_get_active(check));

  g_key_file_set_boolean(config_gfile, PCK_REPEAT,
                         gtk_toggle_button_get_active(check));
...
// Input change events
void input_changed_save_handler(GtkEditable *editable,
                                struct _MainAppWindow *mainappwindow) {
...
gboolean open_safe_mode_dialog() {
  GtkDialog *dialog = gtk_message_dialog_new(
      NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, "Warning");
  gtk_message_dialog_format_secondary_text(
      dialog,
      "Intervals under 100 milliseconds is restricted because of safe mode.");
...
/**
 * Grab click options from ui and pass them to click_opts, then start
 * click_handler.
 * @see click_handler
 */
void start_clicked() {
  ...
  data->sleep = sleep;
  const gchar *mousebutton_text =
      gtk_entry_get_text(GTK_ENTRY(mainappwindow.mouse_entry));
  ...
  const gchar *click_type_text =
      gtk_entry_get_text(GTK_ENTRY(mainappwindow.click_type_entry));
  ...
  if ((data->repeat = gtk_toggle_button_get_active(
           GTK_TOGGLE_BUTTON(mainappwindow.repeat_only_check))))
    data->repeat_times = get_text_to_int(mainappwindow.repeat_entry);

  if ((data->custom_location = gtk_toggle_button_get_active(
  ...
  if ((data->random_interval = gtk_toggle_button_get_active(
           GTK_TOGGLE_BUTTON(mainappwindow.random_interval_check))))
    data->random_interval_ms =
        get_text_to_int(mainappwindow.random_interval_entry);

  if ((data->hold_time = gtk_toggle_button_get_active(
           GTK_TOGGLE_BUTTON(mainappwindow.hold_time_check))))
  ...
  if ((data->check_colour = gtk_toggle_button_get_active(
           GTK_TOGGLE_BUTTON(mainappwindow.colour_check)))) {
    gtk_color_chooser_get_rgba(mainappwindow.colour_entry, &data->colour);
    // printf("data->colour->red = %f\n", data->colour.red);
    // printf("data->colour->red = %f\n", data->colour.blue);
    // printf("data->colour->red = %f\n", data->colour.green);
  }
...
void mainappwindow_import_config() {
  ...
  gtk_entry_set_text_if_not_null(mainappwindow.millisecs_entry,
                                 config->millisecs);

  gtk_entry_set_text_if_not_null(mainappwindow.mouse_entry,
                                 config->mouse_button);
  gtk_entry_set_text_if_not_null(mainappwindow.click_type_entry,
                                 config->click_type);
  gtk_entry_set_text_if_not_null(mainappwindow.hotkey_type_entry,
                                 config->hotkey);
  gtk_toggle_button_set_active(mainappwindow.repeat_only_check,
                               config->use_repeat);
  gtk_entry_set_text_if_not_null(mainappwindow.repeat_entry,
                                 config->repeat_times);

  gtk_toggle_button_set_active(mainappwindow.custom_location_check,
                               config->use_custom_location);
  ...
  gtk_toggle_button_set_active(mainappwindow.random_interval_check,
                               config->use_random_interval);
  gtk_entry_set_text_if_not_null(mainappwindow.random_interval_entry,
                                 config->random_interval_ms);
  gtk_toggle_button_set_active(mainappwindow.hold_time_check,
                               config->use_hold_time);
  gtk_entry_set_text_if_not_null(mainappwindow.hold_time_entry,
                                 config->hold_time_ms);
}

void settings_clicked() { settings_dialog_new(); }

void get_button_clicked() {
  isChoosingLocation = TRUE;
  toggle_get_active(mainappwindow.get_button);
  g_thread_new("get_cursor_pos_click_handler", get_cursor_pos_click_handler,
               NULL);
  g_thread_new("get_cursor_pos_handler", get_cursor_pos_handler,
               mainappwindow.get_button);
}

void colour_picker_clicked() {
  isChoosingLocation = TRUE;
  toggle_get_active(mainappwindow.colour_picker);
  g_thread_new("get_cursor_pos_click_handler",
               get_cursor_pos_click_handler_colour,
               mainappwindow.colour_picker);
}
...
void hotkey_type_entry_changed() {
  const gchar *hotkey_type_text =
      gtk_entry_get_text(GTK_ENTRY(mainappwindow.hotkey_type_entry));
...
void mouse_button_entry_changed() {
  const gchar *mouse_button_entry_text =
      gtk_entry_get_text(GTK_ENTRY(mainappwindow.mouse_entry));

  g_key_file_set_string(config_gfile, PCK_MOUSE_BUTTON,
                        mouse_button_entry_text);
  g_key_file_save_to_file(config_gfile, configpath, NULL);
}

void toggle_clicking(int evtype) {
  if (strcmp(gtk_entry_get_text(GTK_ENTRY(mainappwindow.hotkey_type_entry)),
             "Normal")) {
    if (evtype == KeyPress) {
      start_clicked();
    } else {
...
void get_start_stop_key_handler() {
  ...
  while (1) {
    ...
    if (keyState.button == config->button1 ||
      ...
      isHolding1 =
          keyState.button == config->button1 && keyState.evtype == KeyPress;
      isHolding2 =
          keyState.button == config->button2 && keyState.evtype == KeyPress;
    }
  }
...
void set_start_stop_button_hotkey_text() {
  ...
  if (config->button1 != -1) {
    ...
    start_text = malloc(1 + strlen(start_text_1) + strlen(button_1_key) +
                        strlen(button_2_key) + 4);
    stop_text = malloc(1 + strlen(stop_text_1) + strlen(button_1_key) +
                       strlen(button_2_key) + 4);
...
void random_interval_check_toggle(GtkToggleButton *self) {
  gtk_widget_set_sensitive(mainappwindow.random_interval_entry,
                           gtk_toggle_button_get_active(self));

  g_key_file_set_boolean(config_gfile, PCK_RANDOM_INTERVAL,
                         gtk_toggle_button_get_active(self));
  save_and_populate_config();
}
...
void hold_time_check_toggle(GtkToggleButton *self) {
  gtk_widget_set_sensitive(mainappwindow.hold_time_entry,
                           gtk_toggle_button_get_active(self));

  g_key_file_set_boolean(config_gfile, PCK_HOLD_TIME,
                         gtk_toggle_button_get_active(self));
  save_and_populate_config();
}

void colour_check_toggle(GtkToggleButton *self) {
  gtk_widget_set_sensitive(mainappwindow.colour_entry,
                           gtk_toggle_button_get_active(self));
  gtk_widget_set_sensitive(mainappwindow.colour_picker,
                           gtk_toggle_button_get_active(self));

  // g_key_file_set_boolean(config_gfile, PCK_HOLD_TIME,
  // gtk_toggle_button_get_active(self)); g_key_file_set_boolean(config_gfile,
  // PCK_HOLD_TIME, gtk_toggle_button_get_active(self));
  // save_and_populate_config();
}
...
static void main_app_window_init(MainAppWindow *win) {
  ...
  mainappwindow.colour_entry = win->colour_entry;
  ...
  mainappwindow.colour_check = win->colour_check;
  ...
  mainappwindow.colour_picker = win->colour_picker;
...
static void main_app_window_class_init(MainAppWindowClass *class) {
  ...
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          repeat_only_check_toggle);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          input_changed_save_handler);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          settings_clicked);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          custom_location_check_toggle);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          get_button_clicked);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          random_interval_check_toggle);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          hold_time_check_toggle);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          colour_check_toggle);
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class),
                                          colour_picker_clicked);

  // Entries
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       hours_entry);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       minutes_entry);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
